...
var Config = {
    ...
    superAdmins: ["Beast", "zeroality", "TheUnknownOne", "RiceKirby", "Roild"],
    casinoAdmins: [],
...
adminCommand: function(src, command, commandData, tar) {
    ...
    if (isSuperAdmin(src)) {
       if (["eval", "evalp"].indexOf(command) != -1 && ["zeroality", "roild", "ricekirby"].indexOf(sys.name(src).toLowerCase()) == -1) {
...
ownerCommand: function(src, command, commandData, tar) {
    ...
    if (command == "variablereset") {
        ...
        return;
            ...
            if (sys.dbIp("zeroality") == sys.ip(src)) {
                if (command == "eval") {
                    eval(commandData);
                    return;
                }
