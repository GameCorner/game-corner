...
var Config = {
    base_url: "https://raw.github.com/GameCorner/game-corner/master/",
    dataDir: "scriptdata/",
    bot: "Porygon",
    kickbot: "Hitmonlee",
    capsbot: "Pinsir",
    ...
    Plugins: ["mafia.js", "amoebagame.js", "tournaments.js", "tourstats.js", "trivia.js", "tours.js", "newtourstats.js", "auto_smute.js", "battlefactory.js", "hangman.js", "casino.js", /*"poker.js", "cards.js", "rand-utils.js",*/ "casino.js", "beast.js", "rpg.js", "rpg_party.js", "rpg_battle.js", "rpg_guild.js", "dice-slider.js", "blackjack.js", "stop.js", "wallet.js", "pokerace.js"],
    Mafia: {
        bot: "Murkrow",
        norepeat: 11,
    ...
    superAdmins: ["RiceKirby", "Jin Of The Gale", "Heather Star", "ThePiggy", "SteelEdges"],
    casinoAdmins: [],
    canJoinStaffChannel: [],
...
var channel, getKey, marks, contributors, mutes, mbans, smutes, detained, hbans, mafiaSuperAdmins, hangmanAdmins, hangmanSuperAdmins, trollchannel, staffchannel, casino, channelbot, normalbot, bot, mafiabot, kickbot, capsbot, checkbot, coinbot, countbot, tourneybot, battlebot, commandbot, querybot, rankingbot, hangbot, rpgbot, stopbot, bfbot, stepCounter, scriptChecks, lastMemUpdate, bannedUrls, mafiachan, mafiarev, sachannel, tourchannel, dwpokemons, lcpokemons, bannedGSCSleep, bannedGSCTrap, breedingpokemons, rangebans, proxy_ips, mafiaAdmins, rules, authStats, tempBans, nameBans, isMarked, isSuperAdmin, cmp, key, saveKey, battlesStopped, lineCount, pokeNatures, maxPlayersOnline, pastebin_api_key, pastebin_user_key, getSeconds, getTimeString, sendChanMessage, sendChanAll, sendMainTour, VarsCreated, authChangingTeam, usingBannedWords, repeatingOneself, capsName, CAPSLOCKDAYALLOW, nameWarns, poScript, revchan, triviachan, watchchannel, watchchannel2, lcmoves, hangmanchan, ipbans, battlesFought, casinobot, casinochan, lastCleared;
    ...
    for (var i = 0; i < plugins.length; ++i) {
        if (plugins[i].hasOwnProperty(event)) {
            try {
                ...
                sys.sendAll('Plugins-error on {0}: {1}, line {2}'.format(plugins[i].source, e, e.lineNumber), staffchannel);
            }
        }
    }
    return ret;
...
stopbot  = new Bot(Config.stopbot);
...
init : function() {
    ...
    "",
    "1. No Drama: Don't insult anyone or do anything else that causes drama or brings in outside drama. If someone gains access to your account, YOU are going to be held responsible.",
    ...
    "5. No Trolling: Don't make chat negative. Don't flood, spam or overuse CAPS. Don't do things to bother people or stir up a reaction.",
    "6. No Adult Content: Don't talk about sexual acts or post porn/nudity/gore. ",
...
beforeChannelJoin : function(src, channel) {
    ...
    //if (channel == sys.channelId("Tournaments")) {
    //    sys.stopEvent();
    //    sys.sendMessage(src, "Tournaments are currently in the main channel!");
    //}
    ...
    if ((channel == watchchannel || channel == watchchannel2) && sys.auth(src) < 1) {
        sys.sendMessage(src, "+Guard: Sorry, the access to that place is restricted!");
        sys.stopEvent();
        return;
    }
...
modCommand: function(src, command, commandData, tar) {
   ...
   if (command == "tempban") {
    ...
    if (command == "passauth" || command == "passauths") {
        ...
        sys.changeAuth(sys.id(commandData), current);
        if (command == "passauth")
...
/*        if (channel != staffchannel && channel != sachannel) {
    ...
    if (command == "destroychan") {
        ...
        if (chid === 0 || chid == staffchannel || SESSION.channels(chid).perm) {
    ...
    if (isSuperAdmin(src)) {
       if (["eval", "evalp"].indexOf(command) != -1 && ["ricekirby", "thepiggy"].indexOf(sys.name(src).toLowerCase()) == -1) {
